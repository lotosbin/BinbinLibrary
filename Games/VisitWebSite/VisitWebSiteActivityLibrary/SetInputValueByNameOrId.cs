...
{
    [Designer(typeof(SetInputValueDesigner))]
    public sealed class SetInputValueByNameOrId : CodeActivity
    {
        ...
        {
            string inputName = InputName.Get(context);
            string inputId = InputId.Get(context);
            ...
            foreach (IHTMLElement child in children)
            {
                ...
                {
                    if (IsTargetInput(inputElement, inputName, inputId))
                    {
                ...
                if (textAreaElement != null
                    ...
                    && inputElement.name.Trim().ToUpper() == inputName.Trim().ToUpper())
                    textAreaElement.value = Value.Get(context);

                IHTMLSelectElement selectElement = child as IHTMLSelectElement;

                if (selectElement != null && selectElement.name.Trim().ToUpper() == inputName.Trim().ToUpper())//_name.Trim().ToUpper())
                {
        ...
        private bool IsTargetInput(IHTMLInputElement inputElement, string inputName, string inputId)
        {
            if (inputElement.name == null) return false;
            if (inputElement.name.Trim().ToUpper() == inputName.Trim().ToUpper()) return true;
            if (inputElement.name.Trim().ToUpper() == inputId.Trim().ToUpper()) return true;
            return false;
        }
    }
}